...
float _ColorAdjustHue;
float _ColorAdjustSaturation;
float _ColorAdjustValue;

float3 ForkRGBtoHSV(float3 c) {
...
float3 ForkHSVtoRGB(float3 c) {
	float4 K = float4(1.0, 2.0 / 3.0, 1.0 / 3.0, 3.0);
	float3 p = abs(frac(c.xxx + K.xyz) * 6.0 - K.www);
	return c.z * lerp(K.xxx, clamp(p - K.xxx, 0.0, 1.0), c.y);
}

float3 ForkColorAdjust(float3 color) {
	color = ForkRGBtoHSV(color);
	color.x += _ColorAdjustHue;
	color = ForkHSVtoRGB(color);
	color = lerp(color, dot(color, float3(0.3, 0.59, 0.11)), -_ColorAdjustSaturation);
	color *= 1 + _ColorAdjustValue;
